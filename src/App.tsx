            ...
            return (
              <button
                ...
                onClick={() => {
                  ...
                  if (item.id !== "home") {
                    setMessage(`${item.label} 页面将在下一阶段接入。`);
                  }
                }}
              >
                <Icon size={21} />
        ...
        <div className="modal-backdrop">
          <div className="plan-modal">
            <div className="modal-header">
              <div>
                <h2>{plan.mode === "copy" ? "复制整理确认" : "移动整理确认"}</h2>
                <p>
                  视频 {plan.summary.videos} 个，字幕 {plan.summary.subtitles} 个，冲突 {plan.summary.conflicts} 个
                </p>
              </div>
                ...
                <div className="plan-item" key={`${item.source}-${index}`}>
                  ...
                  {item.collision ? (
                    <select
                      ...
                      onChange={(event) => updateCollision(index, event.target.value as CollisionAction)}
                    >
            ...
            <div className="modal-actions">
              ...
              <button className="primary" onClick={executePlan}>
                确认执行
              </button>
            </div>
          </div>
...
function CandidateChips({ item }: { item: EpisodeMatch }) {
  ...
  return (
    <div className="candidate-chip-row">
      {chips.map((chip) => (
        <span className={`mini-chip ${chip.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`} key={chip}>
          {chip}
        </span>
      ))}
    </div>
